export const heroData = {
  title: 'রঙিনবাড়ি আইটি-তে আপনাকে স্বাগতম',
  subtitle:
    'আপনার স্বপ্ন পূরণে আমরা আছি আপনার পাশে। দেশের সেরা প্রশিক্ষকদের সাথে নিজের স্কিলকে নিয়ে যান এক নতুন উচ্চতায়।',
  image: 'https://placehold.co/1920x1080.png',
  ctaText: 'আমাদের কোর্সসমূহ দেখুন',
  ctaLink: '#courses',
};

export const courses = [
  {
    id: 1,
    title: 'সম্পূর্ণ ওয়েব ডেভেলপমেন্ট',
    description:
      'এইচটিএমএল, সিএসএস, জাভাস্ক্রিপ্ট, রিয়্যাক্ট এবং নোড.জেএস শিখে একজন ফুল-স্ট্যাক ওয়েব ডেভেলপার হোন।',
    image: 'https://placehold.co/600x400.png',
    dataAiHint: 'web development',
    price: '৫,০০০ ৳',
    students: 120,
    lessons: 45,
    link: '#',
  },
  {
    id: 2,
    title: 'গ্রাফিক ডিজাইন ও ফ্রিল্যান্সিং',
    description:
      'অ্যাডোবি ফটোশপ, ইলাস্ট্রেটর এবং ফিগমা ব্যবহার করে আকর্ষণীয় ডিজাইন তৈরি করুন এবং ফ্রিল্যান্সিং মার্কেটে কাজ করুন।',
    image: 'https://placehold.co/600x400.png',
    dataAiHint: 'graphic design',
    price: '৪,৫০০ ৳',
    students: 95,
    lessons: 38,
    link: '#',
  },
  {
    id: 3,
    title: 'ডিজিটাল মার্কেটিং',
    description:
      'এসইও, এসএমএম, এবং কন্টেন্ট মার্কেটিং শিখে ডিজিটাল দুনিয়ায় আপনার ব্যবসাকে প্রসারিত করুন।',
    image: 'https://placehold.co/600x400.png',
    dataAiHint: 'digital marketing',
    price: '৪,০০০ ৳',
    students: 150,
    lessons: 55,
    link: '#',
  },
  {
    id: 4,
    title: 'অ্যান্ড্রয়েড অ্যাপ ডেভেলপমেন্ট',
    description:
      'কোটলিন এবং জাভা ব্যবহার করে অ্যান্ড্রয়েড অ্যাপ তৈরি করা শিখুন এবং প্লে স্টোরে আপনার অ্যাপ প্রকাশ করুন।',
    image: 'https://placehold.co/600x400.png',
    dataAiHint: 'app development',
    price: '৬,০০০ ৳',
    students: 80,
    lessons: 60,
    link: '#',
  },
];

export const aboutUsData = {
  title: 'আমরা কারা?',
  heading: 'আপনার সফলতার পথে, আমরা আপনার বিশ্বস্ত সঙ্গী।',
  description:
    'রঙিনবাড়ি আইটি একটি শীর্ষস্থানীয় আইটি প্রশিক্ষণ কেন্দ্র যেখানে আমরা শিক্ষার্থীদের প্রযুক্তিগত জ্ঞান এবং দক্ষতার মাধ্যমে তাদের স্বপ্ন পূরণে সহায়তা করি। আমাদের লক্ষ্য হল দেশের প্রতিটি প্রান্তে মানসম্মত আইটি শিক্ষা পৌঁছে দেওয়া এবং একটি ডিজিটাল বাংলাদেশ গঠনে ভূমিকা রাখা।',
  image: 'https://placehold.co/800x600.png',
  dataAiHint: 'team meeting',
  stats: [
    { value: '5k+', label: 'সফল শিক্ষার্থী' },
    { value: '10+', label: 'দক্ষ প্রশিক্ষক' },
    { value: '20+', label: 'কোর্সসমূহ' },
  ],
};

export const testimonials = [
  {
    id: 1,
    name: 'আরিফুল ইসলাম',
    role: 'ওয়েব ডেভেলপার',
    quote:
      'রঙিনবাড়ি আইটি থেকে ওয়েব ডেভেলপমেন্ট কোর্সটি করে আমি আজ একজন সফল ফ্রিল্যান্সার। এখানকার প্রশিক্ষকরা খুবই আন্তরিক।',
    avatar: 'https://placehold.co/100x100.png',
    dataAiHint: 'man portrait',
  },
  {
    id: 2,
    name: 'সুমাইয়া আক্তার',
    role: 'গ্রাফিক ডিজাইনার',
    quote:
      'গ্রাফিক ডিজাইন কোর্সটি আমার ক্যারিয়ারের মোড় ঘুরিয়ে দিয়েছে। এখন আমি আন্তর্জাতিক মার্কেটে কাজ করছি। ধন্যবাদ রঙিনবাড়ি আইটি।',
    avatar: 'https://placehold.co/100x100.png',
    dataAiHint: 'woman portrait',
  },
  {
    id: 3,
    name: 'রাকিবুল হাসান',
    role: 'ডিজিটাল মার্কেটার',
    quote:
      ' এখানকার ডিজিটাল মার্কেটিং কোর্সটি খুবই কার্যকরী। আমি নিজের ব্যবসাকে অনলাইন প্ল্যাটফর্মে সফলভাবে প্রতিষ্ঠিত করতে পেরেছি।',
    avatar: 'https://placehold.co/100x100.png',
    dataAiHint: 'person smiling',
  },
];

export const blogPosts = [
  {
    id: 1,
    title: 'ফ্রিল্যান্সিং-এ সফল হওয়ার উপায়',
    excerpt:
      'ফ্রিল্যান্সিং জগতে সফলতা পেতে হলে কিছু নির্দিষ্ট কৌশল অবলম্বন করা জরুরি। এই ব্লগে আমরা সেই কৌশলগুলো নিয়ে আলোচনা করেছি।',
    image: 'https://placehold.co/600x400.png',
    dataAiHint: 'laptop desk',
    date: 'জুলাই ২৫, ২০২৪',
    link: '#',
  },
  {
    id: 2,
    title: 'কেন শিখবেন ওয়েব ডেভেলপমেন্ট?',
    excerpt:
      'বর্তমান সময়ে ওয়েব ডেভেলপমেন্ট একটি চাহিদা সম্পন্ন পেশা। এর ভবিষ্যৎ সম্ভাবনা এবং ক্যারিয়ার গড়ার উপায় জানতে পড়ুন।',
    image: 'https://placehold.co/600x400.png',
    dataAiHint: 'code screen',
    date: 'জুলাই ২০, ২০২৪',
    link: '#',
  },
  {
    id: 3,
    title: 'ডিজিটাল মার্কেটিং এর গুরুত্ব',
    excerpt:
      'ব্যবসার প্রসারে ডিজিটাল মার্কেটিং এর ভূমিকা অপরিসীম। কিভাবে ডিজিটাল মার্কেটিং আপনার ব্যবসাকে এগিয়ে নিতে পারে তা জানুন।',
    image: 'https://placehold.co/600x400.png',
    dataAiHint: 'marketing chart',
    date: 'জুলাই ১৫, ২০২৪',
    link: '#',
  },
];
