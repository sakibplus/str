...
import path from 'path'; // path might not be needed but can be used for basename
import { z } from 'zod';
import content from '@/lib/content.json'; // Directly import the JSON data
...
async function seedDataInternal() {
  ...
  if (!projectId || !token) {
    const errorMsg = 'Sanity environment variables (projectId, token) must be defined.';
  ...
  console.log('Starting data import from Genkit flow...');
...
const seedSanityDataFlow = ai.defineFlow(
  ...
  async () => {
    try {
      const resultMessage = await seedDataInternal();
